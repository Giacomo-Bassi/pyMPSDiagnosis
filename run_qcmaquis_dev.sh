...
if [ "$DO_DMRG" = true ]; then
        cp ${FOLDER}/${FCIDUMP_NAME}.FCIDUMP ${CALC_DIR}/current.FCIDUMP
        cp ${FOLDER}/${KIND}_template.inp ${CALC_DIR}/${PRJ_NAME}.inp
        ...
        if [ "$INITIAL_GUESS" = "chkp" ]; then
                ...
                cp -r ${FOLDER}/${CHKP_NAME} ${CALC_DIR}/current.checkpoint
...
bond_dim_list=(64 128 256 512 1024)
